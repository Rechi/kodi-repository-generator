...
def pack_artifact(artifact, src_dir, dst_dir, linkdir = None):
    ...
    if linkdir == None or dst_dir == linkdir:
        ...
        existsFunction = lambda src, _ = None: os.path.exists(os.path.join(src_dir, src))
        copyFunction = lambda src, dst: shutil.copyfile(os.path.join(src_dir, src), os.path.join(dst_dir, dst))
    else:
        existsFunction = lambda src, srcNew = None: (srcNew != None and os.path.exists(os.path.join(linkdir, srcNew))) or os.path.exists(os.path.join(linkdir, src))
        copyFunction = lambda src, dst: os.link(os.path.join(linkdir, dst), os.path.join(dst_dir, dst))
    ...
    if assets is not None:
        ...
        if existsFunction("icon.png"):
            copyFunction("icon.png", "icon.png")

        if existsFunction("fanart.jpg"):
            copyFunction("fanart.jpg", "fanart.jpg")

        if existsFunction("changelog.txt", "changelog-%s.txt" % artifact.version):
